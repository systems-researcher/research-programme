...
The dependency graph is hand-drawn SVG in `app/src/components/graph.tsx`.

Node positions are computed in Python (`scripts/render.py` `graph()`) with
longest-path layering so every edge flows left to right, then shipped as
integer coordinates on `data/map.json`. The React component does not run a
layout library; it converts those coordinates to pixels.

Strand colour reaches the SVG through CSS variables (for example
`--strand-*-line` on node chrome), so light and dark schemes follow the same
token path as the rest of the page. No chart-library theme object and no
inline `fill:` rules owned by a renderer.
...
| 2026-08-20 | The dependency graph is positioned in Python and drawn as hand-written SVG | Auto-layout cannot be told what a picture means: it produced the diagram this replaced. Longest-path layering guarantees every edge flows left to right, which is what makes the graph readable without a legend. |
| 2026-08-20 | The third strand is Formalisation; the final stage is Release | Both were called Assembly, then both Release, so the row and the column repeated one word. The strand is a line of work — turning findings into the record; the stage is where a study ends up. |
| 2026-08-20 | The written column is `publications`, not `Thesis-Work-Area` | The thesis area is a personal workspace, not programme output. `publications` is the written record: one frozen report per study. |
